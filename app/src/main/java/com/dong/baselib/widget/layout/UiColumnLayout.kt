...
) : LinearLayout(context, attrs, defStyleAttr)
{
    ...
    /** Override onLayout to manually space children */
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        val space = when (justifyContent) {
            3 -> (totalHeight - totalChildrenHeight) / (childCount - 1) // space-between
            4 -> (totalHeight - totalChildrenHeight) / childCount // space-around
            5 -> (totalHeight - totalChildrenHeight) / (childCount + 1) // space-evenly
            else -> 0
        }

        if (justifyContent == 5) currentY += space // Adjust for space-evenly
